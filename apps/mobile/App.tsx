...
import { fetchNextQuestion, submitAttempt, type PracticeQuestion as ApiQuestion } from './src/api';
import {
  Pressable,
  SafeAreaView,
  ScrollView,
...
export default function App() {
  ...
  const [activeQuestion, setActiveQuestion] = useState<PracticeQuestion | ApiQuestion | null>(null);
  ...
  const masteryMap = useMemo(
    () => Object.fromEntries(concepts.map((c) => [c.id, c.mastery])),
    [concepts],
  );

  const loadQuestionFromApi = useCallback(async () => {
    setLoadingQuestion(true);
    try {
      const q = await fetchNextQuestion(masteryMap);
      setActiveQuestion(q);
      setApiConnected(true);
    } catch {
      setApiConnected(false);
      setActiveQuestion(practiceQuestions[activeQuestionIndex] ?? practiceQuestions[0]);
    } finally {
      setLoadingQuestion(false);
    }
  }, [masteryMap, activeQuestionIndex]);

  useEffect(() => {
    void loadQuestionFromApi();
  }, [loadQuestionFromApi]);

  const currentQuestion = activeQuestion ?? practiceQuestions[activeQuestionIndex];
  const activeConcept = concepts.find((concept) => concept.id === currentQuestion.conceptId);
  ...
  const shownHints = currentQuestion.hints.slice(0, hintsRevealed);
  const canRevealHint = hintsRevealed < currentQuestion.hints.length;
  ...
  const submitAnswer = async () => {
    ...
    if (!trimmedResponse) {
      setFeedback('Write your reasoning first. Padhaaku grades the thinking path, not just a final answer.');
      return;
    }

    let score: number;
    let explanation = currentQuestion.explanation;
    const focusPoint = 'focusPoint' in currentQuestion ? currentQuestion.focusPoint : '';
    ...
    if (apiConnected) {
      try {
        ...
        score = getScoreFromResponse(
          trimmedResponse,
          'answerKeywords' in currentQuestion ? currentQuestion.answerKeywords : [],
        );
      }
    } else {
      score = getScoreFromResponse(
    ...
    const conceptName = activeConcept?.name ?? currentQuestion.subject;
    ...
    setFeedback(
      score >= 8
        ? `Mastery signal: ${score}/10. Strong reasoning. ${explanation}`
        : `Mastery signal: ${score}/10. Review the scaffold: ${focusPoint || explanation}`,
    );
  };
        ...
        <View style={styles.practiceCard}>
          <View style={styles.practiceHeader}>
            ...
            <View style={styles.difficultyBadge}>
              <Text style={styles.difficultyText}>D{currentQuestion.difficulty}</Text>
            </View>
          </View>
          ...
          <View style={styles.focusBox}>
            ...
            <Text style={styles.focusText}>
              {'focusPoint' in currentQuestion ? currentQuestion.focusPoint : 'Hybrid RAG selected this question for your weakest concept.'}
            </Text>
          </View>
